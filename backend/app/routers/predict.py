...
from backend.app.history_store import (  # noqa: F401  imported so tests can monkeypatch it
    HISTORY_PATH,
    get_history,
    save_prediction,
)
...
def _update_model_memory_gauge() -> None:
    ...
    try:
        ...
        if pt_model is not None:
            param_bytes = sum(
                p.numel() * p.element_size() for p in pt_model.parameters()
            )
...
async def predict(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        # ── Drift detection ───────────────────────────────────────────────────
        feature_vec = frames_tensor.mean(
            dim=0
        ).numpy()  # (3, 224, 224) — matches baseline shape
        ...
        if pytorch_model is not None:
            gradcam_b64 = (
                explainability.generate_gradcam(pytorch_model, frames_tensor) or ""
            )
        ...
        PREDICTION_COUNTER.labels(
            label=prediction, mode=mode, source_type=source_type
        ).inc()
        IMAGES_PROCESSED.labels(mode=mode, status="success").inc()
        REQUEST_COUNT.labels(
            method="POST", endpoint="/predict", status="200", mode=mode
        ).inc()
        ...
        REQUEST_COUNT.labels(
            method="POST", endpoint="/predict", status="4xx", mode=mode
        ).inc()
        raise
    except Exception as e:
        REQUEST_COUNT.labels(
            method="POST", endpoint="/predict", status="500", mode=mode
        ).inc()
...
def submit_feedback(body: FeedbackRequest):
    ...
    try:
        ...
        logger.error(
            "feedback_log_failed",
            extra={"error": str(e), "request_id": body.request_id},
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def predict_batch(request: Request, files: List[UploadFile] = File(...)):
    ...
    if len(files) > 10:
        raise HTTPException(
            status_code=400, detail="Maximum 10 files per batch request"
        )
...
async def demo_start(request: Request):
    ...
    if has_used_demo(ip):
        raise HTTPException(
            status_code=403, detail="Demo already used from this IP address"
        )
    record_demo_use(ip)
