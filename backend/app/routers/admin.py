...
def platform_stats():
    ...
    latencies = [
        ...
        for r in records
        if r.get("inference_latency_ms", 0) > 0
    ]
    ...
    fb_f1 = (
        (2 * fb_precision * fb_recall / (fb_precision + fb_recall))
        if fb_precision and fb_recall
        else None
    )
    ...
    if run_id and run_id != "unknown":
        ...
        try:
            resp = httpx.get(
                f"{mlflow_url}/api/2.0/mlflow/runs/get?run_id={run_id}", timeout=3.0
            )
            if resp.status_code == 200:
                metrics_list = (
                    resp.json().get("run", {}).get("data", {}).get("metrics", [])
                )
    ...
    return {
        ...
        "mlflow_val_loss": (
            round(mlflow_metrics["val_loss"], 4)
            if "val_loss" in mlflow_metrics
            else None
        ),
    }
