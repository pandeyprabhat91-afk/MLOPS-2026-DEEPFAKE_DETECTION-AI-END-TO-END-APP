...
def isolate_tickets(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "backend.app.routers.support.TICKETS_PATH", str(tmp_path / "tickets.json")
    )
    monkeypatch.setattr(
        "backend.app.ticket_store.TICKETS_PATH", str(tmp_path / "tickets.json")
    )
...
def test_get_tickets_admin():
    client.post(
        "/support/tickets",
        json={"subject": "Bug", "description": "detail"},
        headers={"X-Username": "user1"},
    )
...
def test_get_tickets_non_admin_sees_only_own():
    client.post(
        "/support/tickets",
        json={"subject": "My bug", "description": "detail"},
        headers={"X-Username": "user1"},
    )
    resp = client.get(
        "/support/tickets", headers={"X-Username": "user1", "X-Role": "user"}
    )
    assert resp.status_code == 200
    for t in resp.json():
...
def test_resolve_nonexistent_ticket():
    resp = client.patch(
        "/support/tickets/TK-FAKEID/resolve",
        json={"resolution": "N/A"},
        headers={"X-Role": "admin"},
    )
...
def test_chat_returns_reply_for_known_keyword():
    resp = client.post(
        "/support/chat", json={"message": "What file formats are supported?"}
    )
    assert resp.status_code == 200
