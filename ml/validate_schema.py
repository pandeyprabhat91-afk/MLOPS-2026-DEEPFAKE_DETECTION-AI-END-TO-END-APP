...
def validate_feature_file(path: str) -> List[str]:
    ...
    if not (MIN_FRAMES <= num_frames <= MAX_FRAMES):
        errors.append(
            f"{p.name}: num_frames={num_frames} outside [{MIN_FRAMES}, {MAX_FRAMES}]"
        )
    if C != EXPECTED_CHANNELS:
        errors.append(f"{p.name}: channels={C}, expected {EXPECTED_CHANNELS}")
    if H != EXPECTED_HEIGHT or W != EXPECTED_WIDTH:
        errors.append(
            f"{p.name}: spatial dims=({H},{W}), expected ({EXPECTED_HEIGHT},{EXPECTED_WIDTH})"
        )

    stem = p.stem.lower()
    if "real" not in stem and "fake" not in stem:
        errors.append(
            f"{p.name}: filename must contain 'real' or 'fake' to encode label"
        )
