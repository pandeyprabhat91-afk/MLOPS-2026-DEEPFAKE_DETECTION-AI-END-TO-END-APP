...
from ml.data_loader import get_dataloaders  # noqa: E402
from ml.model import DeepfakeDetector  # noqa: E402
...
def extract_cnn_features(model, samples, device, aug_copies: int = 3):
    ...
    """
    print(
        f"Pre-extracting CNN features for {len(samples)} videos (aug_copies={aug_copies})..."
    )
    model.eval()
    cached = []
    with torch.no_grad():
    ...
    print(
        f"Feature extraction complete. {len(cached)} total samples (was {len(samples)}). Shape: {cached[0][0].shape}"
    )
    return cached
...
def run_training(samples, params_file: str = "ml/params.yaml"):
    ...
    with mlflow.start_run() as run:
        ...
        optimizer = Adam(
            filter(lambda p: p.requires_grad, model.parameters()), lr=params["lr"]
        )
        ...
        mlflow.pytorch.log_model(
            model, artifact_path="model", registered_model_name="deepfake"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--params_file", type=str, default="ml/params.yaml", help="Path to params.yaml"
    )
    ...
    print("Metrics written to ml/metrics.json")
